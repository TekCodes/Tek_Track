...
public class User {
    ...
    @Column(name = "first_name", length = 50) // Maps the `fName` field to the "first_name" column in the database, with a maximum length of 50 characters.
    private String fName; // Defines the `fName` field to store the user's first name.

    @Column(name = "last_name", length = 50) // Maps the `lName` field to the "last_name" column in the database, with a maximum length of 50 characters.
    private String lName; // Defines the `lName` field to store the user's last name.

    @Column(length = 254, unique = true) // Maps the `email` field to a column in the database with a maximum length of 254 characters and ensures uniqueness.
